...
*{{box-sizing:border-box;letter-spacing:0}}body{{margin:0;background:#f4f7fb;color:#172033;font-family:Vazirmatn,Tahoma,sans-serif}}main{{max-width:1100px;margin:auto;padding:24px 16px 50px}}header{{display:flex;justify-content:space-between;align-items:center;margin-bottom:20px}}h1{{font-size:25px;margin:0}}h2{{font-size:18px;margin:0 0 16px}}.card{{background:#fff;border:1px solid #e2e8f0;border-radius:8px;padding:20px;margin-bottom:16px;box-shadow:0 8px 24px rgba(15,23,42,.05)}}.grid{{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:13px}}label{{display:grid;gap:6px;color:#64748b;font-size:13px}}input,textarea{{border:1px solid #cbd5e1;border-radius:8px;padding:11px;font:inherit;color:#172033;min-width:0}}textarea{{min-height:88px;resize:vertical}}button{{border:0;border-radius:8px;background:{panel.primary_color};color:white;padding:11px 16px;font:inherit;font-weight:700;cursor:pointer}}.danger{{background:#dc2626;padding:7px 10px}}.wide{{grid-column:1/-1}}.notice,.error{{padding:11px;border-radius:8px;margin-bottom:16px;overflow-wrap:anywhere}}.notice{{background:#dcfce7;color:#166534}}.error{{background:#fee2e2;color:#991b1b}}a{{color:{panel.primary_color};font-weight:700}}.actions{{display:flex;justify-content:flex-end;margin-top:14px}}.toggle,.tiny-toggle{{display:flex;align-items:center;gap:8px}}.sub-list{{display:grid;gap:12px}}.sub-card{{border:1px solid #e2e8f0;border-radius:8px;padding:14px;background:#f8fafc;display:grid;gap:12px}}.sub-card-head{{display:flex;justify-content:space-between;gap:12px;align-items:flex-start}}.sub-card-head div{{display:grid;gap:4px;min-width:0}}.sub-card-head span{{color:#64748b;font-size:12px}}.sub-card-head b{{white-space:nowrap;color:{panel.primary_color}}}.sub-links{{display:flex;flex-wrap:wrap;gap:8px;align-items:center}}.sub-grid{{display:grid;grid-template-columns:minmax(0,1fr) minmax(0,1.2fr);gap:10px}}.inline-form,.stack-form{{display:grid;gap:8px;align-content:start}}.inline-form{{grid-template-columns:minmax(0,1fr) auto;align-items:end}}.inline-form button,.stack-form button,.copy-admin{{padding:8px 11px}}.copy-admin{{background:#334155}}.toggle-row{{display:flex;flex-wrap:wrap;gap:12px}}.break{{overflow-wrap:anywhere;white-space:normal}}.ltr{{direction:ltr;text-align:left}}details{{background:#fff;border:1px solid #e2e8f0;border-radius:8px;padding:8px}}summary{{cursor:pointer;color:#64748b}}.delete-form{{display:flex;justify-content:flex-end}}.empty-admin{{padding:16px;color:#64748b;text-align:center}}@media(max-width:700px){{main{{padding:18px 10px 40px}}header{{display:grid;gap:10px}}.grid,.sub-grid,.inline-form{{grid-template-columns:1fr}}.wide{{grid-column:auto}}.sub-card-head{{display:grid}}}}</style></head><body><main><header><div><h1>مدیریت Phantom Subscription</h1><span>ساخته‌شده بر پایه ظاهر marzban-template</span></div><a href="{settings.public_base_url}/health">وضعیت سرویس</a></header>{flash}
...
<section class="card"><h2>لینک‌های ثبت‌شده</h2><div class="sub-list">{rows}</div></section></main><script>async function copyAdminLink(value){{try{{await navigator.clipboard.writeText(value);alert('لینک کپی شد.')}}catch(error){{prompt('برای کپی لینک:',value)}}}}</script></body></html>"""
